...
VCFsummary(snps_cand)# 264 individuals and 611 SNPs.

#F. Saving the subsetted VCF:
Save(snps_cand, "./sPCA/GEA/Pilocarpus_All_SNPs_Candidates.vcf")

#G. Create a Genind object based on VCFR
vcf_cand = read.vcfR("./sPCA/GEA/Pilocarpus_All_SNPs_Candidates.vcf", verbose = FALSE)
vcf_cand
...
write.csv(cbind(mySpca$ls,coordinates), "./sPCA/GEA/Pilocarpus_sPCA_Axis_QGIS.csv")

###5.2. SAVE RESULTS AS GRAPHS AND TABLES
#A. Tables
results_sPCA = summary(mySpca)
write.csv(results_sPCA,"./sPCA/GEA/Pilocarpus_sPCA_Summary.csv")

#B. Spatial and Inertial Components:
pdf("./sPCA/GEA/Spatial_Inertia.pdf", onefile =F)
plot.new()
fgraph(mySpca)
dev.off()

#C. Barplot with Global and Local Structure in spectral color pattern
pdf("./sPCA/GEA/Global_Local_Structure_Spectral.pdf", onefile =F)
...
pdf("./sPCA/GEA/Global_Local_Structure_Significant.pdf", onefile =F)
barplot(mySpca$eig, main="Eigenvalues of sPCA", col=rep(c("red","grey"),c(n_posi,n_eig)))
dev.off()
...
variables = 2
pdf("./sPCA/GEA/Colorplot_Significant_PC.pdf", onefile =F)
