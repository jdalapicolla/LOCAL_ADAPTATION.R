...
# PC1_var = "aridityIndexThornthwaite" "wc2.1_30s_bio_2"
# PC2_var = "SB"
# PC3_var = "H_Al"
...
mod.lfmm = lfmm_lasso( Y = pilocarpus_geno, #Sparse LFMM
                         X = pred_scale, 
                         K = 4)  ## K is the number of populations
...
adj.PC1 <- pchisq(zs[,1]^2/2, df = 1, lower = FALSE)
...
adj.PC2 <- pchisq(zs[,2]^2/1.05, df = 1, lower = FALSE) 
...
adj.PC3 <- pchisq(zs[,3]^2/1.5, df = 1, lower = FALSE) 
...
###Save Results:
pdf("GEA/LFMM/GEA_Outliers_GIF_Original_PC1.pdf", onefile = T)
hist(p.PC1, main="Histogram of p-values PC1")
dev.off()

pdf("GEA/LFMM/GEA_Outliers_GIF_Calibrated_PC1.pdf", onefile = T)
hist(adj.PC1, main="Histogram of adjusted p-values PC1")
dev.off()

pdf("GEA/LFMM/GEA_Outliers_GIF_Original_PC2.pdf", onefile = T)
hist(p.PC2, main="Histogram of p-values PC2")
dev.off()

pdf("GEA/LFMM/GEA_Outliers_GIF_Calibrated_PC2.pdf", onefile = T)
hist(adj.PC2, main="Histogram of adjusted p-values PC2")
dev.off()

pdf("GEA/LFMM/GEA_Outliers_GIF_Original_PC3.pdf", onefile = T)
hist(p.PC3, main="Histogram of p-values PC3")
dev.off()

pdf("GEA/LFMM/GEA_Outliers_GIF_Calibrated_PC3.pdf", onefile = T)
hist(adj.PC2, main="Histogram of adjusted p-values PC3")
dev.off()

#### Save filtered vcf
Save(snps_fil_lfmm_candidate_all, "GEA/LFMM/snps_candidate_lfmm_GEA_pilocarpus.vcf")
...
write.fasta(sequences = SEQ1, names = names(SEQ1), nbchar = 150, file.out = "GEA/LFMM/SEQ1_lfmm_pilocarpus_GEA.fasta")
write.fasta(sequences = SEQ2, names = names(SEQ2), nbchar = 150, file.out = "GEA/LFMM/SEQ2_lfmm_pilocarpus_GEA.fasta")
write.fasta(sequences = SEQ3, names = names(SEQ3), nbchar = 150, file.out = "GEA/LFMM/SEQ3_lfmm_pilocarpus_GEA.fasta")
write.fasta(sequences = total, names = names(total), nbchar = 150, file.out = "GEA/LFMM/SEQ_total_lfmm_pilocarpus_GEA.fasta")
